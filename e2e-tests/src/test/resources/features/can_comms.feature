...
Feature: CAN Communications Serialization/Deserialization
  ...
  # -- CAN Queue Pointer Wrap-around --
  # Merged from can_queue_wrap.feature (C1).
  # Tests can_common.h circular buffer pointer wrap in can_pop, can_push, can_slots_empty.
  # Uses direct queue state manipulation via JNA to reach edge conditions without
  # pushing/popping hundreds of messages through USB endpoints.

  Scenario: CAN queue — r_ptr wraps to 0 when can_pop reads last element
    Given exists data:
      """
      ControlSetup: {
        hwType: 3
  ...
  Scenario: CAN queue — w_ptr wraps to 0 (next_w_ptr) when can_push at end of queue
    Given exists data:
      """
      ControlSetup: {
        hwType: 3
  ...
  Scenario: CAN queue — can_push fails when queue is full (w_ptr at end, r_ptr at 0)
    Given exists data:
      """
      ControlSetup: {
        hwType: 3
  ...
  Scenario: CAN queue — can_slots_empty returns correct count when w_ptr < r_ptr (wrap)
    Given exists data:
      """
      ControlSetup: {
        hwType: 3
  ...
  Scenario: CAN queue — can_slots_empty with w_ptr >= r_ptr (non-wrap) for regression
    Given exists data:
      """
      ControlSetup: {
        hwType: 3
